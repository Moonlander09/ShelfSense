...
<AnimatePresence>
  {open && (
    ...
    >
      <motion.div
        ...
        className="w-full max-w-[420px] bg-white rounded-2xl shadow-xl px-4 py-6 border border-slate-300"
        ...
        {user ? (
          <div className="flex relative flex-col gap-6">
            ...
            <button
              type="button"
              onClick={() => setOpen(false)}
              className="absolute right-0 top-0 flex h-6 w-6 items-center justify-center rounded-full bg-slate-100 border border-slate-200 hover:text-[var(--expired)] cursor-pointer hover:scale-125 hover:border-[var(--expired)] hover:bg-[var(--expired)]/15 transition-all duration-250"
            >
            ...
            {/* User info */}
            <div className="flex items-center gap-3 px-3 py-3 rounded-2xl border border-slate-200 bg-slate-50">
  ...
  {/* Icon */}
  <div className="flex h-11 w-11 items-center justify-center rounded-full bg-slate-100 border border-slate-200 text-[var(--fresh)] animate-pulse">
    <FiUser className="h-5 w-5" />
  </div>
  ...
  <div>
    ...
    <p className="text-sm text-slate-500 mt-1 max-w-xs">
      Your personal pantry tracker to reduce waste and stay organized.
    </p>
  </div>

  {/* Feature hints */}
  <div className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600 space-y-1.5">
    <div>• Organize items into clear categories</div>
<div>• Add, edit, and delete items anytime</div>
<div>• Manage your inventory in one place</div>
  </div>
