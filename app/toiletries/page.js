...
export default function ToiletriesItemCards() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["toiletries"],
    queryFn: () => getItemsByCategory("toiletries"),
  });

  // For Editing Item

  const { mutate: editMutate, isPending: isEditItemPending } = useMutation({
    mutationKey: ["editToiletriesItem"],
  ...
  const { mutate: deleteMutate, isPending: isDeleteItemPending } = useMutation({
    mutationKey: ["deleteToiletriesItem"],
  ...
  return (
    <>
      ...
      <div className="px-2 py-8">
        ...
        >
          <h1 className="text-lg font-semibold">Toiletries</h1>
